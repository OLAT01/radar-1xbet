...
// On garde tes ligues favorites mais on autorise tout en mode Live si besoin
const ELITE_IDS = [39, 40, 41, 61, 140, 141, 78, 79, 135, 88, 94, 253, 2, 3, 1];

export default function ProBetV16_8() {
  ...
  const forceFetch = async (section: string) => {
    ...
    try {
      // 1. RADARS (LIVE) : On prend TOUS les lives du monde sans filtrer par ligue pour ne plus être vide
      ...
      // Filtrage Radar Historique (15-42 min + au moins 1 but)
      const histo = live.filter((m: any) => 
        m.fixture.status.elapsed >= 15 && 
        m.fixture.status.elapsed <= 42 && 
        (m.goals.home + m.goals.away) >= 1
      );
      setRadarHisto(histo.slice(0, 8));

      // Filtrage Radar Danger (45-88 min + score live présent)
      const danger = live.filter((m: any) => 
        m.fixture.status.elapsed >= 45 && 
        m.fixture.status.elapsed < 88
      );
      setRadarDanger(danger.slice(0, 8));

      // 2. COUPONS (MATCHS DU JOUR)
      if (section === 'sauveur' || section === 'coupon' || section === 'all') {
        ...
        // On cherche d'abord dans l'élite, sinon on prend les matchs importants du jour
        let filtered = all.filter((m: any) => ELITE_IDS.includes(m.league.id) && m.fixture.status.short === "NS");
        if (filtered.length === 0) filtered = all.filter((m: any) => m.fixture.status.short === "NS").slice(0, 20);

        setLeSauveur(filtered.slice(0, 2));
        setSingleCoupon(filtered.slice(2, 6));
      }
    } catch (e) { console.error("API Out"); }
    setLoading(null);
  };
  ...
  return (
    <div className="min-h-screen bg-[#0a0f1d] text-slate-200 pb-24 font-sans uppercase font-bold tracking-tighter">
      <header className="bg-[#11192e] p-4 border-b border-slate-800 sticky top-0 z-50 flex justify-between items-center shadow-2xl">
        <div className="flex items-center gap-2 text-red-600 font-black"><Target /><span className="text-lg italic">PROBET V16.8 GOLD</span></div>
        <button onClick={() => forceFetch('all')} className="p-2 bg-red-600/10 rounded-full border border-red-600/20 active:bg-red-600/40">
      ...
      <main className="max-w-md mx-auto p-4 space-y-8">
        ...
        <section className="space-y-3">
          <h3 className="text-red-600 text-[10px] flex items-center gap-2 px-1 font-black"><ShieldCheck size={14}/> LE SAUVEUR (PRÉVISIONS)</h3>
            ...
            return (
              <div key={i} className={`bg-[#11192e] border-t-4 ${op.border} p-4 rounded-xl shadow-xl`}>
                ...
                <div className="grid grid-cols-2 gap-2 text-[9px] text-center italic font-bold">
                  <div className="bg-blue-600/10 p-2 rounded text-blue-400 border border-blue-600/10">+7.5 CORNERS</div>
                  <div className="bg-yellow-600/10 p-2 rounded text-yellow-500 border border-yellow-600/10">+2.5 CARTONS</div>
                </div>
              </div>
            );
          }) : <div className="p-6 text-center text-slate-700 text-[10px] italic bg-[#11192e] rounded-xl border border-slate-900 font-black">CHARGEMENT DES PRÉVISIONS...</div>}
        </section>

        {/* SECTION 2: COUPON UNIQUE */}
        <section className="bg-[#11192e] p-5 rounded-3xl border border-slate-800 shadow-2xl">
          <h3 className="text-slate-500 text-[10px] italic font-black mb-4">COUPON DU {new Date().toLocaleDateString('fr-FR')}</h3>
          <div className="space-y-4">
            {singleCoupon.map((m, i) => {
              const op = getLogic(m);
              return (
                <div key={i} className="border-l-2 pl-3" style={{borderColor: op.col}}>
                  <div className="flex justify-between text-[10px] font-black text-white uppercase"><span>{m.teams.home.name}</span><span>{m.teams.away.name}</span></div>
        ...
        {/* SECTION 3: RADAR HISTORIQUE (SCORE LIVE) */}
        <section className="bg-sky-600/5 border border-sky-600/20 rounded-xl overflow-hidden shadow-xl">
          <div className="bg-sky-600/10 p-3 text-sky-400 text-[9px] font-black flex justify-between items-center">
             <div className="flex items-center gap-2 font-black"><Clock size={14}/> RADAR HISTORIQUE (15'-42' +BUT)</div>
             <RefreshCw size={12} className={loading === 'histo' ? "animate-spin" : ""}/>
          </div>
          <div className="divide-y divide-slate-800">
            {radarHisto.length > 0 ? radarHisto.map((m, i) => (
              <div key={i} className="p-3 flex justify-between items-center text-[11px] text-white bg-[#11192e]/20 font-black">
                <span className="truncate max-w-[150px] font-black">{m.teams.home.name}-{m.teams.away.name}</span>
                <span className="text-yellow-500 font-black">{m.goals.home}-{m.goals.away} ({m.fixture.status.elapsed}')</span>
              </div>
            )) : <div className="p-8 text-center text-slate-700 text-[9px] italic font-black">AUCUN MATCH DANS LA ZONE (15'-42')</div>}
          </div>
        </section>

        {/* SECTION 4: RADAR DANGER (SCORE LIVE + MAX 88') */}
        <section className="bg-orange-600/5 border border-orange-600/20 rounded-xl overflow-hidden shadow-xl font-black">
          <div className="bg-orange-600/10 p-3 text-orange-400 text-[9px] font-black flex justify-between items-center">
             <div className="flex items-center gap-2 font-black"><Flame size={14}/> RADAR DANGER (MAX 88')</div>
             <RefreshCw size={12} className={loading === 'danger' ? "animate-spin" : ""}/>
          </div>
          {radarDanger.length > 0 ? radarDanger.map((m, i) => (
            <div key={i} className="p-4 border-b border-slate-800 flex flex-col gap-2 bg-[#11192e]/40 last:border-0 font-black">
              <div className="flex justify-between items-center text-[11px] font-black">
                <span className="truncate max-w-[170px] text-white font-black">{m.teams.home.name} - {m.teams.away.name}</span>
                <span className="text-orange-500 text-[15px] font-black">{m.goals.home} - {m.goals.away}</span>
              </div>
              <div className="flex justify-between items-center font-black">
                <span className="text-[10px] text-slate-500 italic font-black uppercase">PRESSION LIVE</span>
          ...
          )) : <div className="p-8 text-center text-slate-700 text-[9px] italic font-black">RECHERCHE DE MATCHS EN 2ÈME MI-TEMPS...</div>}
        </section>
      </main>
    </div>
  );
