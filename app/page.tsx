...
export default function ProBetV12_5() {
  ...
  const fetchInitialData = useCallback(async () => {
    try {
      ...
      // Setup initial des sections
      updateRadarHistorique(all);
      updateRadarDanger(all);
  ...
  // LOGIQUE RADAR HISTORIQUE INDÉPENDANTE
  const updateRadarHistorique = (matches: any[]) => {
  ...
  // LOGIQUE RADAR DANGER INDÉPENDANTE
  const updateRadarDanger = (matches: any[]) => {
    const danger = matches.filter((m: any) => ["1H", "2H"].includes(m.fixture.status.short) && GOLD_LEAGUES.includes(m.league?.id)).slice(0, 5);
    setRadarDanger(danger);
  };

  // ACTIONS DES BOUTONS (VRAI SYNC SÉPARÉ)
  const refreshSection = async (type: string) => {
    ...
    try {
      ...
      if (type === 'HISTO') updateRadarHistorique(all);
      if (type === 'DANGER') updateRadarDanger(all);
      if (type === 'SAUVEUR') setLeSauveur(all.filter((m: any) => m.fixture.status.short === "NS").sort(() => Math.random() - 0.5).slice(0, 2));
      if (type === 'COUPON') setSingleCoupon(all.filter((m: any) => m.fixture.status.short === "NS").sort(() => Math.random() - 0.5).slice(3, 6));
  ...
  return (
    <div className="min-h-screen bg-[#0a0f1d] text-slate-200 pb-24 font-sans">
      <header className="bg-[#11192e] p-4 flex items-center border-b border-slate-800 sticky top-0 z-50">
        ...
        <span className="font-black text-xl italic uppercase">ProBet V12.5</span>
      </header>
      ...
      <main className="max-w-md mx-auto p-4 space-y-8">
        ...
        <section className="space-y-4">
          <div className="flex justify-between items-center px-1">
            ...
            <button onClick={() => refreshSection('SAUVEUR')} className="text-slate-500 p-2"><RefreshCw className={`w-4 h-4 ${loadingRadar === 'SAUVEUR' ? 'animate-spin' : ''}`} /></button>
          </div>
          {leSauveur.map((m, i) => (
        ...
        <section className="bg-[#11192e] rounded-2xl p-5 border border-slate-800 shadow-2xl">
          <div className="flex justify-between items-center mb-6">
            <h3 className="text-xs font-black uppercase text-slate-400 italic font-sans">Unique Coupon</h3>
            <button onClick={() => refreshSection('COUPON')} className="text-slate-500 p-2"><RefreshCw className={`w-4 h-4 ${loadingRadar === 'COUPON' ? 'animate-spin' : ''}`} /></button>
          </div>
        ...
        <section className="bg-sky-500/5 border border-sky-500/20 rounded-2xl overflow-hidden shadow-lg">
          <div className="bg-sky-500/10 p-3 flex justify-between items-center text-sky-400 font-black text-[10px] uppercase border-b border-sky-500/20">
            ...
            <button onClick={() => refreshSection('HISTO')} className="bg-sky-500/20 p-2 rounded-full hover:bg-sky-500/40 active:scale-90 transition-all">
              <RefreshCw className={`w-4 h-4 ${loadingRadar === 'HISTO' ? 'animate-spin' : ''}`} />
            </button>
          </div>
        ...
        <section className="bg-orange-500/5 border border-orange-500/20 rounded-2xl overflow-hidden shadow-lg">
          <div className="bg-orange-500/10 p-3 flex justify-between items-center text-orange-400 font-black text-[10px] uppercase border-b border-orange-500/20">
            ...
            <button onClick={() => refreshSection('DANGER')} className="bg-orange-500/20 p-2 rounded-full hover:bg-orange-500/40 active:scale-90 transition-all">
              <RefreshCw className={`w-4 h-4 ${loadingRadar === 'DANGER' ? 'animate-spin' : ''}`} />
            </button>
          </div>
          <div className="divide-y divide-slate-800">
            {radarDanger.map((m, i) => (
