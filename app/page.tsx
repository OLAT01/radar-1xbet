...
export default function ProBetV13_0() {
  ...
  const getProPrediction = (m: any, level: 'STRICT' | 'NORMAL') => {
    ...
    if (level === 'STRICT') {
        if ([39, 78, 140].includes(leagueId)) return { tip: "DOUBLE CHANCE : 1X", col: "#4ade80" };
        return { tip: "LES DEUX ÉQUIPES MARQUENT : OUI", col: "#f472b6" };
    }
    if (leagueId === 39) return { tip: "CORNERS : +8.5", col: "#38bdf8" };
    return { tip: "TOTAL : +1.5 BUTS", col: "#fbbf24" };
  };

  // FONCTION DE RAFRAÎCHISSEMENT ULTRA-CIBLÉE
  const refreshSection = async (section: 'SAUVEUR' | 'COUPON' | 'HISTO' | 'DANGER') => {
    ...
    try {
      ...
      const elite = rawMatches.filter((m: any) => ELITE_LEAGUES.includes(m.league.id));
      
      const nowTs = new Date().getTime();

      // ICI ON NE MODIFIE QUE LA SECTION DEMANDÉE
      if (section === 'SAUVEUR') {
        const secure = elite.filter((m: any) => m.fixture.status.short === "NS" && [39, 140, 78, 135].includes(m.league.id))
                           .sort(() => Math.random() - 0.5).slice(0, 2);
        setLeSauveur(secure);
      } 
      else if (section === 'COUPON') {
        const coup = elite.filter((m: any) => m.fixture.status.short === "NS")
                          .sort(() => Math.random() - 0.5).slice(3, 6);
        setSingleCoupon(coup);
      } 
      else if (section === 'HISTO') {
        const hist = elite.filter((m: any) => {
        ...
        const danger = elite.filter((m: any) => ["1H", "2H"].includes(m.fixture.status.short)).slice(0, 5);
  ...
  useEffect(() => {
    if (!isLoaded) {
      const init = async () => {
        await Promise.all([
          refreshSection('SAUVEUR'),
          refreshSection('COUPON'),
          refreshSection('HISTO'),
          refreshSection('DANGER')
        ]);
        setIsLoaded(true);
      };
      init();
  ...
  return (
    <div className="min-h-screen bg-[#0a0f1d] text-slate-200 pb-24 font-sans">
      <header className="bg-[#11192e] p-4 border-b border-slate-800 sticky top-0 z-50 flex justify-between items-center">
        <div className="flex items-center"><Trophy className="text-yellow-500 w-6 h-6 mr-2" /><span className="font-black text-xl italic uppercase">ProBet V13</span></div>
        <div className="bg-green-500/10 text-green-500 text-[9px] px-2 py-1 rounded border border-green-500/20 font-bold">1XBET ELITE MODE</div>
      </header>
      ...
      <main className="max-w-md mx-auto p-4 space-y-8">
        ...
        <section className="bg-orange-500/5 border border-orange-500/20 rounded-2xl overflow-hidden shadow-lg">
          ...
          <div className="divide-y divide-slate-800">
            {radarDanger.map((m, i) => (
              <div key={i} className="p-4 bg-orange-500/[0.03] flex justify-between items-center font-sans">
