...
export default function ProBetV17_6() {
  ...
  const fetchData = async (section: string = "all") => {
    ...
    try {
      ...
      // Utilisation de live=all pour garantir d'avoir des matchs en direct
  ...
  // --- LOGIQUE DES RADARS (SÉCURISÉE) ---
  const radarHisto = fixtures.filter(m => {
    const elapsed = m.fixture.status.elapsed || 0;
    const goals = (m.goals.home ?? 0) + (m.goals.away ?? 0);
    // Règle : 15' à 42' ET au moins 1 but
    return elapsed >= 15 && elapsed <= 42 && goals >= 1;
  }).slice(0, 7);
  ...
  // --- SAUVEUR & COUPON (94% PROBA) ---
  // On prend les matchs qui ont le plus de buts en cours (indicateur de forme)
  const topSelections = [...fixtures].sort((a, b) => 
  ...
  return (
    <div className="min-h-screen bg-[#020617] text-white p-4 font-sans uppercase font-bold tracking-tighter pb-10">
      <header className="flex justify-between items-center mb-6 border-b border-slate-800 pb-4">
        <div className="flex items-center gap-2 text-red-600 italic font-black text-xl"><Target /> PROBET V17.6</div>
      ...
      <main className="space-y-6 max-w-md mx-auto">
        ...
        <section className="bg-red-950/20 border border-red-600/30 rounded-3xl p-5 relative shadow-2xl">
          ...
          {topSelections.length > 0 ? topSelections.slice(0, 2).map((m, i) => (
            <div key={i} className="bg-slate-900/80 p-3 rounded-xl mb-2 border-l-4 border-red-600">
               ...
               <div className="text-center text-red-500 text-[11px] mt-1 font-black">DOUBLE CHANCE & +1.5 BUTS</div>
            </div>
          )) : <div className="animate-pulse text-slate-800 text-[9px] text-center">RECHERCHE...</div>}
        </section>
        ...
        <section className="bg-emerald-950/20 border border-emerald-500/30 rounded-3xl p-5 relative shadow-2xl">
          ...
          <div className="space-y-2">
            {topSelections.length > 0 ? topSelections.slice(2, 6).map((m, i) => (
            ...
            )) : <div className="animate-pulse text-slate-800 text-[9px] text-center">ANALYSE...</div>}
          </div>
        </section>
        ...
        <section className="bg-slate-900/50 border border-sky-500/20 rounded-3xl p-5 relative">
          ...
          <div className="space-y-3">
            ...
            )) : <p className="text-center text-slate-700 text-[9px] py-4 italic">AUCUNE OPPORTUNITÉ BUT 1MT</p>}
          </div>
        </section>
