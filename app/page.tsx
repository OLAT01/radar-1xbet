...
export default function ProBetV12_1() {
  ...
  const refreshRadars = (matches = allMatches, nowTs = new Date().getTime()) => {
    setRadarHistorique(matches.filter((m: any) => {
      ...
      <main className="max-w-md mx-auto p-4 space-y-8">
        ...
        <section className="space-y-4">
          <div className="flex justify-between items-center px-1">
            ...
            <button onClick={() => refreshSauveur()} className="text-slate-500 hover:text-white transition-colors"><RefreshCw className="w-4 h-4" /></button>
          </div>
          {leSauveur.map((m, i) => (
        ...
        <section className="bg-[#11192e] rounded-2xl p-5 border border-slate-800 shadow-2xl">
          <div className="flex justify-between items-center mb-6">
            ...
            <button onClick={() => refreshCoupon()} className="text-slate-500 hover:text-yellow-500 transition-colors"><RefreshCw className="w-4 h-4" /></button>
          </div>
        ...
        <section className="bg-sky-500/5 border border-sky-500/20 rounded-2xl overflow-hidden">
          <div className="bg-sky-500/10 p-3 flex justify-between items-center text-sky-400 font-black text-[10px] uppercase border-b border-sky-500/20">
            ...
            <button onClick={() => refreshRadars()} className="text-sky-400/50 hover:text-sky-400"><RefreshCw className="w-4 h-4" /></button>
          </div>
        ...
        <section className="bg-orange-500/5 border border-orange-500/20 rounded-2xl overflow-hidden shadow-lg">
          <div className="bg-orange-500/10 p-3 flex justify-between items-center text-orange-400 font-black text-[10px] uppercase border-b border-orange-500/20">
            ...
            <button onClick={() => refreshRadars()} className="text-orange-400/50 hover:text-orange-400"><RefreshCw className="w-4 h-4" /></button>
          </div>
          <div className="divide-y divide-slate-800">
            {radarDanger.map((m, i) => (
              <div key={i} className="p-4 bg-orange-500/[0.03]">
                <div className="flex justify-between items-center mb-2">
                  ...
                  <div className="text-[14px] font-black text-orange-500">{m.goals.home} - {m.goals.away}</div>
                </div>
