...
export default function ProBetV11_1() {
  ...
  const getSmartAnalysis = (m: any, index: number) => {
    ...
    if (index === 1) return { tip: "PLUS DE 3.5 CARTONS", col: "#f472b6" };
  ...
  const fetchAllData = useCallback(async () => {
    try {
      ...
      setLoadTime(nowTs); // Enregistre l'heure du clic

      // 1. RADAR HISTORIQUE (Disparition à la 42ème min)
      ...
      // 2. RADAR DANGER (Préparation initiale)
  ...
  // GESTION DU TIMER DE 20 MINUTES POUR LE RADAR DANGER
  useEffect(() => {
    if (isLoaded) {
      const checkExpiry = setInterval(() => {
        const currentTime = new Date().getTime();
        const minutesPassed = (currentTime - loadTime) / 60000;
        
        if (minutesPassed >= 20) {
          setRadarDanger([]); // Vide le radar danger après 20 min
        }
  ...
  return (
    <div className="min-h-screen bg-[#0a0f1d] text-slate-200 pb-24 font-sans">
      <header className="bg-[#11192e] p-4 flex justify-between items-center border-b border-slate-800 sticky top-0 z-50 shadow-xl">
        <div className="flex items-center gap-2"><Trophy className="text-yellow-500 w-6 h-6" /><span className="font-black text-xl italic uppercase font-sans">ProBet</span></div>
        <button onClick={fetchAllData} className="bg-yellow-500 text-black px-5 py-2 rounded-full font-black text-[11px] uppercase flex items-center gap-2 active:scale-90 transition-all shadow-lg shadow-yellow-500/20">
      ...
      <main className="max-w-md mx-auto p-4 space-y-8">
        ...
        <section className="space-y-4">
          ...
          {leSauveur.map((m, i) => (
            <div key={i} className="bg-[#11192e] border border-slate-800 rounded-2xl p-4 border-l-4 border-l-red-500">
              <div className="flex justify-between text-[8px] text-slate-500 mb-2 font-black uppercase font-sans"><span>{m.league.name}</span><span>{new Date(m.fixture.date).toLocaleTimeString([], {hour:'2-digit', minute:'2-digit'})}</span></div>
        ...
        <section className="bg-sky-500/5 border border-sky-500/20 rounded-2xl overflow-hidden shadow-lg">
          ...
          <div className="divide-y divide-slate-800 font-sans">
            ...
            )) : <div className="p-6 text-center text-[10px] text-slate-600 italic uppercase">Aucun match historique en cours</div>}
          </div>
        </section>

        {/* RADAR DANGER (AVEC DISPARITION 20MIN) */}
        <section className="bg-orange-500/5 border border-orange-500/20 rounded-2xl overflow-hidden shadow-lg">
          ...
          <div className="divide-y divide-slate-800">
            {radarDanger.length > 0 ? radarDanger.map((m, i) => (
              <div key={i} className="p-4 bg-orange-500/[0.03] font-sans">
            ...
            )) : <div className="p-6 text-center text-[10px] text-slate-600 italic uppercase italic">Le radar danger s'est vidé. Rafraîchissez pour voir de nouveaux matchs.</div>}
          </div>
        </section>
      </main>
    </div>
