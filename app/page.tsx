...
export default function ProBetV14_5() {
  ...
  const getProPrediction = (m: any, level: 'STRICT' | 'NORMAL') => {
    ...
    if (level === 'STRICT') {
        if ([39, 78, 140].includes(leagueId)) return { tip: "DOUBLE CHANCE : 1X", col: "#4ade80", conf: "94%" };
        return { tip: "LES DEUX ÉQUIPES MARQUENT : OUI", col: "#f472b6", conf: "89%" };
    }
    return { tip: "TOTAL : +1.5 BUTS", col: "#fbbf24", conf: "85%" };
  };
  ...
  return (
    <div className="min-h-screen bg-[#0a0f1d] text-slate-200 pb-24 font-sans">
      <header className="bg-[#11192e] p-4 border-b border-slate-800 sticky top-0 z-50 flex justify-between items-center">
        <div className="flex items-center"><Trophy className="text-yellow-500 w-6 h-6 mr-2" /><span className="font-black text-xl italic uppercase tracking-tighter">ProBet V14.5 Elite</span></div>
        <div className="bg-red-500/10 text-red-500 text-[9px] px-2 py-1 rounded border border-red-500/20 font-bold uppercase">Analyse Hybride Active</div>
      </header>
      ...
      <main className="max-w-md mx-auto p-4 space-y-8">
        ...
        <section className="space-y-4">
          ...
          {leSauveur.map((m, i) => (
            <div key={i} className="bg-[#11192e] border border-slate-800 rounded-2xl p-4 border-l-4 border-l-red-500 shadow-xl">
              <div className="flex justify-between text-[8px] text-slate-500 mb-2 font-black uppercase"><span>{m.league.name}</span><span className="text-yellow-500 flex items-center gap-1"><BadgeCheck className="w-3 h-3" /> Certifié {getProPrediction(m, 'STRICT').conf}</span></div>
              ...
              <div className="text-[10px] text-red-500 font-black bg-red-500/5 py-2 text-center rounded-lg border border-red-500/20 uppercase mb-3">{getProPrediction(m, 'STRICT').tip}</div>
              <a href={getAnalysisLink(m.teams.home.name, m.teams.away.name, 'FOOTY')} target="_blank" className="w-full bg-slate-800/50 p-2 rounded-lg flex items-center justify-center gap-2 text-[8px] font-bold hover:bg-slate-700 transition"><BarChart2 className="w-3 h-3 text-blue-400" /> VÉRIFIER STATS H2H (FOOTYSTATS)</a>
            </div>
          ))}
        </section>

        {/* UNIQUE COUPON (3 MATCHS) - RÉTABLI */}
        ...
        {/* RADAR HISTORIQUE - RÉTABLI AVEC TEMPS ÉCOULÉ */}
        ...
        {/* RADAR DANGER - RÉTABLI AVEC PACKBALL */}
          ...
          <div className="divide-y divide-slate-800">
            {radarDanger.map((m, i) => (
              <div key={i} className="p-4 bg-orange-500/[0.03] flex flex-col gap-3">
                ...
                <a href={getAnalysisLink(m.teams.home.name, m.teams.away.name, 'PACK')} target="_blank" className="w-full py-2 bg-orange-500/10 border border-orange-500/20 rounded flex items-center justify-center gap-2 text-[8px] font-black text-orange-400 hover:bg-orange-500/20 transition uppercase"><Zap className="w-3 h-3" /> VOIR PRESSION LIVE (PACKBALL)</a>
              </div>
            ))}
          </div>
        </section>
