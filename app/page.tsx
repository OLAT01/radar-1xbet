...
export default function ProBetV16_5() {
  ...
  const forceFetch = async (section: string) => {
    ...
    try {
      ...
      setRadarHisto(live.filter((m: any) => m.fixture.status.elapsed >= 15 && m.fixture.status.elapsed <= 42 && (m.goals.home + m.goals.away) >= 1));
      setRadarDanger(live.filter((m: any) => m.fixture.status.elapsed >= 45 && m.fixture.status.elapsed < 88));
  ...
  return (
    <div className="min-h-screen bg-[#0a0f1d] text-slate-200 pb-24 font-sans uppercase font-bold tracking-tighter">
      <header className="bg-[#11192e] p-4 border-b border-slate-800 sticky top-0 z-50 flex justify-between items-center shadow-2xl">
        <div className="flex items-center gap-2 text-red-600"><Target /><span className="text-lg italic tracking-tighter">PROBET V16.5 GOLD</span></div>
      ...
      <main className="max-w-md mx-auto p-4 space-y-8">
        <section className="space-y-3">
          ...
          }) : <div className="p-6 text-center text-slate-600 text-[10px] italic">RECHERCHE DE MATCHS ELITE...</div>}
        </section>

        <section className="bg-[#11192e] p-5 rounded-3xl border border-slate-800 shadow-xl">
          <div className="flex justify-between items-center mb-4 px-1">
            <h3 className="text-slate-500 text-[10px] italic">COUPON {new Date().toLocaleDateString('fr-FR')}</h3>
        ...
        <section className="bg-sky-600/5 border border-sky-600/20 rounded-xl overflow-hidden shadow-xl text-white">
          ...
          <div className="divide-y divide-slate-800 font-black">
            ...
            )) : <div className="p-6 text-center text-slate-600 text-[9px] italic">AUCUN MATCH EN DIRECT (15'-42')</div>}
          </div>
        </section>
        ...
        <section className="bg-orange-600/5 border border-orange-600/20 rounded-xl overflow-hidden shadow-xl text-white">
          ...
          )) : <div className="p-6 text-center text-slate-600 text-[9px] italic">AUCUN MATCH EN DIRECT (45'-88')</div>}
        </section>
      </main>
    </div>
  );
